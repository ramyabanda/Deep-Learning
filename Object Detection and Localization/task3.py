...
Original file is located at
    https://colab.research.google.com/drive/16QRzL4aKiJZXKXskFHeuRMRNxKTPd0xm

Disclaimer: Obtained the DLStudio code form Professor Kak's Homepage at https://pypi.org/project/DLStudio/1.0.7/

For Task 3, although unsuccessful my approach to implement is as follows:



> For the first half of task3, I merged the four noisy datasets into one each for training and testing using a custom_dataset() method I wrote. I also labelled these datasets ranging accordingly. This dataset was then supposed to be worked into a custom cnn to be able to classify the percentage of noise in an image. For this purpose, I built a cnn method called cnn() taking inspiration from the vgg-16 network (https://neurohive.io/en/popular-networks/vgg16/). 


> For the second half, although unsuccessful due to a number of errors, I had created the models for the respective noisy datasets. If the above were a success, the output of the CNN would classify the image (batch_size = 1) as belonging to a noise category. This image would then further be sent into the respective model created. The idea behind being that, a model solely trained on a certain noise level can better detect the shape and localize better.
"""
...
######################################################################################################################################
######################### CUSTOM DATASET WITH ALL NOISE DATASETS MERGED AS ONE TRAINING AND ONE TESTING SET ##########################
######################################################################################################################################             
              
              def custom_dataset(self, train_or_test, dataset_file1, dataset_file2, dataset_file3, dataset_file4):
                ...
                if train_or_test == 'train':
                  if os.path.exists("torch-saved-PurdueShapes5-10000-dataset.pt") and \
                                ...
                                while(idx < len(self.dataset)):
                                  self.dataset_train_new[idx][0] = np.array(self.dataset[idx][0])
                                  self.dataset_train_new[idx][1] = np.array(self.dataset[idx][1])
                                  self.dataset_train_new[idx][2] = np.array(self.dataset[idx][2])
                                  self.dataset_train_new[idx][3] = 0
                                  self.dataset_train_new[idx][4] = 0
                  ...
                  if os.path.exists("torch-saved-PurdueShapes5-10000-dataset-noise-20.pt") and \
                                ...
                                while(idx < 20000):
                                  ...
                                  self.dataset_train_new[idx][1] = np.array(self.dataset[idx][1])
                                  self.dataset_train_new[idx][2] = np.array(self.dataset[idx][2])
                                  self.dataset_train_new[idx][3] = 1
                                  self.dataset_train_new[idx][4] = 1
                                  idx += 1
                  else: 
                          print("""\n\n\nLooks like this is the first time you will be loading in\n"""
                                """the dataset for this script. First time loading could take\n"""
                  ...
                  if os.path.exists("torch-saved-PurdueShapes5-10000-dataset-noise-50.pt") and \
                                ...
                                while(idx < 30000):
                                  ...
                                  self.dataset_train_new[idx][1] = np.array(self.dataset[idx][1])
                                  self.dataset_train_new[idx][2] = np.array(self.dataset[idx][2])
                                  self.dataset_train_new[idx][3] = 2
                                  self.dataset_train_new[idx][4] = 2
                                  idx += 1
                  else: 
                          print("""\n\n\nLooks like this is the first time you will be loading in\n"""
                                """the dataset for this script. First time loading could take\n"""
                  ...
                  if os.path.exists("torch-saved-PurdueShapes5-10000-dataset-noise-80.pt") and \
                                ...
                                while(idx < 40000):
                                  ...
                                  self.dataset_train_new[idx][1] = np.array(self.dataset[idx][1])
                                  self.dataset_train_new[idx][2] = np.array(self.dataset[idx][2])
                                  self.dataset_train_new[idx][3] = 3
                                  self.dataset_train_new[idx][4] = 3
                                  idx += 1
                  else: 
                          print("""\n\n\nLooks like this is the first time you will be loading in\n"""
                                """the dataset for this script. First time loading could take\n"""
                  ...
                  if os.path.exists("torch-saved-PurdueShapes5-10000-dataset.pt") and \
                                ...
                                idx=0
                                while(idx < len(self.dataset)):
                                  self.dataset_test_new[idx][0] = np.array(self.dataset[idx][0])
                                  self.dataset_test_new[idx][1] = np.array(self.dataset[idx][1])
                                  self.dataset_test_new[idx][2] = np.array(self.dataset[idx][2])
                                  self.dataset_test_new[idx][3] = 0
                                  self.dataset_test_new[idx][4] = 0
                  ...
                  if os.path.exists("torch-saved-PurdueShapes5-10000-dataset-noise-20.pt") and \
                                ...
                                while(idx < 2000):
                                  ...
                                  self.dataset_test_new[idx][1] = np.array(self.dataset[idx][1])
                                  self.dataset_test_new[idx][2] = np.array(self.dataset[idx][2])
                                  self.dataset_test_new[idx][3] = 1
                                  self.dataset_test_new[idx][4] = 1
                                  idx += 1
                  else: 
                          print("""\n\n\nLooks like this is the first time you will be loading in\n"""
                                """the dataset for this script. First time loading could take\n"""
                  ...
                  if os.path.exists("torch-saved-PurdueShapes5-10000-dataset-noise-50.pt") and \
                                ...
                                while(idx < 3000):
                                  ...
                                  self.dataset_test_new[idx][1] = np.array(self.dataset[idx][1])
                                  self.dataset_test_new[idx][2] = np.array(self.dataset[idx][2])
                                  self.dataset_test_new[idx][3] = 2
                                  self.dataset_test_new[idx][4] = 2
                                  idx += 1
                  else: 
                          print("""\n\n\nLooks like this is the first time you will be loading in\n"""
                                """the dataset for this script. First time loading could take\n"""
                  ...
                  if os.path.exists("torch-saved-PurdueShapes5-10000-dataset-noise-80.pt") and \
                                ...
                                while(idx < 4000):
                                  ...
                                  self.dataset_test_new[idx][1] = np.array(self.dataset[idx][1])
                                  self.dataset_test_new[idx][2] = np.array(self.dataset[idx][2])
                                  self.dataset_test_new[idx][3] = 3
                                  self.dataset_test_new[idx][4] = 3
                                  idx += 1
                  else: 
                          print("""\n\n\nLooks like this is the first time you will be loading in\n"""
                                """the dataset for this script. First time loading could take\n"""
                      ...
                      # for idx1 in range(count*10000,(count*10000)+len(self.dataset)):
                      #       self.dataset_test_new[idx1][0] = np.array(self.dataset[idx1][0])
                      #       self.dataset_test_new[idx1][1] = np.array(self.dataset[idx1][1])
                      #       self.dataset_test_new[idx1][2] = np.array(self.dataset[idx1][2])
                      #       if(count == 0):
                      #         self.dataset_test_new[idx1][3] = 0
                      #         self.dataset_test_new[idx1][4] = 0
                      #       elif(count == 1):
                      #         self.dataset_test_new[idx1][3] = 1
                      #         self.dataset_test_new[idx1][4] = 1
                      #       elif(count == 2):
                      #         self.dataset_test_new[idx1][3] = 2
                      #         self.dataset_test_new[idx1][4] = 2
                      #       else:
                      #         self.dataset_test_new[idx1][3] = 3
                      #         self.dataset_test_new[idx1][4] = 3
                      # count += 1
...
############################################################################################################################################
##### CNN model built taking inspiration from the vgg-16 network as showin in https://neurohive.io/en/popular-networks/vgg16/ ##############
############################################################################################################################################

          class cnn(nn.Module):

              def __init__(self):
                  super(DetectAndLocalize.cnn, self).__init__()
                  ...
                  self.pool = nn.MaxPool2d(2, 2)
                  self.dense1 = nn.Linear(7*7*512, 4096)
  ...
  ##################################################################################################################  
  ################################ Training and Testing set Merged Dataset #########################################
  ##################################################################################################################


  dataserver_merged_train = DetectAndLocalize.PurdueShapes5Dataset.custom_dataset(
                                      train_or_test = 'train',dataset_file1 = "PurdueShapes5-20-train.gz", dataset_file2 = "PurdueShapes5-10000-train-noise-20.gz", dataset_file3 = "PurdueShapes5-10000-train-noise-50.gz", dataset_file4 = "PurdueShapes5-10000-train-noise-80.gz")
  dataserver_merged_test = DetectAndLocalize.PurdueShapes5Dataset.custom_dataset(
                                      train_or_test = 'test',dataset_file1 = "PurdueShapes5-20-test.gz", dataset_file2 = "PurdueShapes5-1000-test-noise-20.gz", dataset_file3 = "PurdueShapes5-1000-test-noise-50.gz", dataset_file4 = "PurdueShapes5-1000-test-noise-80.gz")

  
  ##################################################################################################################
  ################################ Model for Merged Noise Dataset ##################################################
  ##################################################################################################################

  
  detector.dataserver_merged_train = dataserver_merged_train
  detector.dataserver_merged_test = dataserver_merged_test
  
  detector.load_PurdueShapes5_dataset(dataserver_merged_train, dataserver_merged_test)
  
  model_merged = detector.LOADnet2(skip_connections=True, depth=32)
  detector.run_code_for_training_with_CrossEntropy_and_MSE_Losses(model_merged)
  detector.run_code_for_testing_detection_and_localization(model_merged, noise = 0)


  ################################ Training and Testing set for No Noise #########################################

  dataserver_0train = DetectAndLocalize.PurdueShapes5Dataset(
                                      train_or_test = 'train',
                                      dl_studio = dls,
  ...
  dataserver_0test = DetectAndLocalize.PurdueShapes5Dataset(
                                      train_or_test = 'test',
                                      dl_studio = dls,
  ...
  ################################ Training and Testing set for 20% Noise #########################################

  dataserver_20train = DetectAndLocalize.PurdueShapes5Dataset(
                                      train_or_test = 'train',
                                      dl_studio = dls,
  #                                   dataset_file = "PurdueShapes5-20-train.gz", 
                                      dataset_file = "PurdueShapes5-10000-train-noise-20.gz", 
  ...
  dataserver_20test = DetectAndLocalize.PurdueShapes5Dataset(
                                      train_or_test = 'test',
                                      dl_studio = dls,
  #                                   dataset_file = "PurdueShapes5-20-test.gz"
                                      dataset_file = "PurdueShapes5-1000-test-noise-20.gz"
  ...
  ################################ Training and Testing set for 80% Noise #########################################

  dataserver_80train = DetectAndLocalize.PurdueShapes5Dataset(
                                      train_or_test = 'train',
                                      dl_studio = dls,
  #                                   dataset_file = "PurdueShapes5-20-train.gz", 
  #                                   dataset_file = "PurdueShapes5-10000-train-noise-20.gz", 
  ...
  dataserver_80test = DetectAndLocalize.PurdueShapes5Dataset(
                                      train_or_test = 'test',
                                      dl_studio = dls,
  #                                   dataset_file = "PurdueShapes5-20-test.gz"
  #                                   dataset_file = "PurdueShapes5-1000-test-noise-20.gz"
  ...
  detector.dataserver_train = dataserver_0train
  detector.dataserver_test = dataserver_0test
  
  detector.load_PurdueShapes5_dataset(dataserver_0train, dataserver_0test)
  model0 = detector.LOADnet2(skip_connections=True, depth=32)
  detector.run_code_for_training_with_CrossEntropy_and_MSE_Losses(model0)
  detector.run_code_for_testing_detection_and_localization(model0, noise = 0)

  ################################ Model for 20% Noise ############################################################ 
  
  detector.dataserver_train = dataserver_20train
  detector.dataserver_test = dataserver_20test
  
  detector.load_PurdueShapes5_dataset(dataserver_20train, dataserver_20test)
  model20 = detector.LOADnet2(skip_connections=True, depth=32)
  detector.run_code_for_training_with_CrossEntropy_and_MSE_Losses(model20)
  detector.run_code_for_testing_detection_and_localization(model20, noise = 20)
  ...
  detector.dataserver_train = dataserver_80train
  detector.dataserver_test = dataserver_80test
  
  detector.load_PurdueShapes5_dataset(dataserver_80train, dataserver_80test)
  model80 = detector.LOADnet2_Smooth(skip_connections=True, depth=32)
  detector.run_code_for_training_with_CrossEntropy_and_MSE_Losses(model80)
  detector.run_code_for_testing_detection_and_localization(model80, noise = 80)
